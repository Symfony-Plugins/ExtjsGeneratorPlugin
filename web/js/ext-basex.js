...
(function() {
  ...
  }, emptyFn = Ext.emptyFn || function() {}, OP = Object.prototype;
  ...
  Ext.extend(Ext.lib.Ajax.Queue, Object, {
     ...
     * 
     * @param {Boolean}
     *          peek If true, the queue item is returned but not removed from
     * the stack. @ default false
     */
    next : function(peek) {
     ...
     * requests until the {@link #Ext.ux.ModuleManager-resume} method is called.
     */
    suspend : function() {
      this.suspended = true;
    },
    ...
    /**
     * Dispatches the next queue item and initiates a Ext.lib.Ajax request on
     * the result.
     * 
     * @param {Boolean}
     *          peek If true, the queue item is returned but not removed from
  ...
  Ext.extend(Ext.lib.Ajax.QueueManager, Object, {
    /**
     * @cfg {Integer} quantas Adjustable milliseconds deferred dispatch timer
     *      interval
     */
    quantas : 10,
    ...
    /**
     * @cfg (Boolean) progressive Default Dispatch mode for all defined queues
     *      <p>
     *      a false value will exhaust a priority queue until empty during
     *      dispatch (sequential)
     *      <p>
     *      true to dispatch a single request from each priority queue until all
     *      queues exhausted.
     *      <p>
    ...
    /**
     * private main Request dispatch loop. This keeps the maximum allowed number
     * of requests going at any one time (based on defined queue priority and
  ...
  Ext.apply(A, {
    ...
    monitoredNode : function(tag, attributes, callback, context, deferred) {
      ...
      var node = null, doc = (context || window).document, head = doc ? doc.getElementsByTagName("head")[0] : null;
      ...
      if (tag && doc && head) {
        ...
        if (callback) {
          var cb = (callback.success || callback).createDelegate(callback.scope || null, [
              callback || {}
            ], 0);
     ...
     *           concurrent Queued browser (XHR) requests Note: IE8 increases
     *           this limit to 6
     */
    ...
    serializeForm : function() {
      ...
      return function(form) {
        var fElements = form.elements || (document.forms[form] || Ext.getDom(form)).elements, hasSubmit = false, encoder = this.encoder, element, options, name, val, data = '', type;
    ...
    getHttpStatus : function(reqObj, isAbort, isTimeout) {
      ...
      } catch (e) {} // status may not avail/valid yet, called too early, or
                      // status not support by the transport
    ...
    handleTransactionResponse : function(o, callback, isAbort, isTimeout) {
      ...
      if (!o.status.isError) {
        ...
        /*
         * create and enhance the response with proper status and XMLDOM if
         * necessary
         */
       ...
       * during XML-DOM creation? And mixin everything the XHR object had to
       * offer as well
       */
    ...
    createResponseObject : function(o, callbackArg, isAbort, isTimeout) {
      ...
      if (isAbort !== true) {
        ...
        if ((o.status.isLocal || o.proxied) && typeof obj.responseText == 'string') {
          ...
          if (o.status.isOK && ((!obj.responseXML && this.reCtypeXML.test(obj.contentType)) || (obj.responseXML && obj.responseXML.childNodes.length === 0))) {
            ...
            try { // ActiveX may be disabled
              if (Ext.capabilities.hasActiveX) {
                ...
                var domParser = null;
                try { // Opera 9 will fail parsing non-XML content, so trap
                      // here.
          ...
          if (obj.responseXML) {
            var parseBad = (obj.responseXML.documentElement && obj.responseXML.documentElement.nodeName == 'parsererror') || (obj.responseXML.parseError || 0) !== 0
        ...
        if (o.options.isJSON || (this.reCtypeJSON && this.reCtypeJSON.test(headerObj[CTYPE] || ""))) {
          try {
            Ext.isObject(obj.responseJSON) || (obj.responseJSON = Ext.isFunction(this.decodeJSON) && Ext.isString(obj.responseText) ? this.decodeJSON(obj.responseText) : null);
          } catch (exJSON) {
            o.status.isError = true; // trigger future exception callback
    ...
    request : function(method, uri, cb, data, options) {
      ...
      if (!this.events || this.fireEvent('request', method, uri, cb, data, O) !== false) {
        ...
        var cType;
        // The Content-Type specified on options.headers always has priority
        // over
        // a calculated value.
      ...
      try {
        if (f = options.proxied) { /* JSONP scriptTag Support */
                ...
                if (this.el) {
                  ...
                  if (!o.debug) {
                    if (Ext.isIE) {
                      ...
                      this.el.remove();
                      // Other Browsers will not GBG-collect these tags, so help
                      // them along
                      if (d) {
                        for (var prop in d) {
                          delete d[prop];
          ...
          o.conn.open = function() {
            ...
            // apply any new params to any already supplied by the uri and
            // postData
            ...
            }, null, f.target, true); // defer head insertion until send method
    ...
    makeRequest : function(method, uri, callback, postData, options) {
      ...
      if (o = this.getConnectionObject(uri, options, postData)) {
        ...
        try {
          ...
          ('onreadystatechange' in r) && (r.onreadystatechange = this.onStateChange.createDelegate(this, [
              o, callback, 'readystate'
            ], 0));

          ('onload' in r) && (r.onload = this.onStateChange.createDelegate(this, [
              o, callback, 'load', 4
            ], 0));

          ('onprogress' in r) && (r.onprogress = this.onStateChange.createDelegate(this, [
              o, callback, 'progress'
            ], 0));
          ...
          if (callback && callback.timeout) {
            ...
            ('ontimeout' in r) && (r.ontimeout = this.abort.createDelegate(this, [
                o, callback, true
              ], 0));
            ('ontimeout' in r) ||
    ...
    onStateChange : function(o, callback, mode) {
      ...
      if (mode === 'load' || readyState > 2) {
        ...
        if (ct && /multipart\//i.test(ct)) {
          ...
          if (p) {
            ...
            forEach(Array.slice(p, o.parts.length), // skip parts already parsed
            function(newPart) {
      ...
      this.events && this.fireEvent.apply(this, [
          'readystatechange'
  ...
  if (Ext.util.Observable) {
    ...
    Ext.apply(A, {
      ...
      /**
       * onStatus define eventListeners for a single (or array) of HTTP status
       * codes.
       */
      ...
      /**
       * unStatus unSet eventListeners for a single (or array) of HTTP status
       * codes.
       */
  ...
  Ext.applyIf(Array.prototype, {
    ...
    /*
     * Fix for IE, Opera < 9.5, which does not seem to include the map function
     * on Array's
     */
    ...
    /**
     * Return true of the passed Function test true of ANY array elememt. (added
     * for IE)
     */
    some : function(fn) {
     ...
     * (http://dean.edwards.name/weblog/2006/07/enum/) Gecko already supports
     * forEach for Arrays : see
  ...
  Ext.apply(caps, {
    ...
    /**
     * @property {Boolean} hasXDR True if the Browser has native Cross-Domain
    ...
    /**
     * @property {Boolean} hasChromeFrame true, if the Google ChromeFrame plugin
     *           is install on IE
     */
    ...
    hasFlash : (function() {
      ...
      if (defined(window.ActiveXObject)) {
        ...
        for (var i = 0, P = navigator.plugins, length = P.length; i < length; ++i) {
          // test to see if any plugin names contain the word flash, if so it
          // must support it - return true
     ...
     *           enabled/supported. On IE, ModalDialog windows will issue a
     *           security risk warning to the user during this check, so assert
     *           to false. (Cookie implementations on IE's
     *           [Modeless|Modal]Dialogs are not supported as they run in a
     *           seperate ActiveX browser context)
     */
    ...
    /**
     * @property {Boolean} hasCanvasText True if the browser has canvas Element
     *           Text support.
     */
    ...
    /**
     * @property {Boolean} hasWorkers True if the browser has support for
     *           threaded Workers.
     */
    ...
    /**
     * @property {Boolean} hasLocalStorage True if the browser has Local Storage
     *           support.
     */
     ...
     * @desc Basic HTML5 Element support for the &lt;audio> tag and/or Audio
     *       object.
     *       @example
     *       If the browser has &lt;audio> tag or Audio object support,<br />
     *       the property contains a mime-type map of standard audio formats. {
     *       mp3 : false, //mp3 ogg : false, //Ogg Vorbis wav : true, //wav
     *       basic : false, //au, snd aif : false, //aif, aifc, aiff tag : true,
     *       //is audio HTML element supported? object : true, //is the
     *       window.Audio Object supported <b>testMime</b> : function() }
     * 
     * The included <b>testMime</b> function permits selective mime-type
     * testing as well for custom audio formats: if(Ext.capabilities.hasAudio &&
     * Ext.capabilities.hasAudio.testMime('audio/ogg') ){ alert ('Vorbis
     * playback is supported'); }
     */
    hasAudio : function() {

      var aTag = !!document.createElement('audio').canPlayType, aAudio = ('Audio' in window) ? new Audio('') : {}, caps = aTag || ('canPlayType' in aAudio) ? {
        tag : aTag,
    ...
    hasVideo : function() {
      var vTag = !!document.createElement('video').canPlayType, vVideo = vTag ? document.createElement('video') : {}, caps = ('canPlayType' in vVideo) ? {
        tag : vTag,
        /*
         * Test for a specific video and codec (eg: 'video/ogg; codecs="theora,
         * vorbis"' )
         */
        ...
        mp4 : 'video/mp4; codecs="avc1.42E01E, mp4a.40.2"', // mp4 (Apple:
                                                            // patented licensed
                                                            // codec)
     ...
     *          testEl (optional) A specific HTMLElement/Object to test against,
     *          otherwise a tagName to test against. based on the passed
     *          eventName is used, or DIV as default. (window and document
...
Ext.applyIf(Function.prototype, {
   ...
   *         which, when called, executes after a specific delay. Optionally, a
   *         replacement (or additional) argument list may be specified.
   * @param {Number}
   *          delay The number of milliseconds to defer execution by whenever
   *          called.
   * @param {Object}
   *          scope (optional) The scope (<code>this</code> reference) used
   *          by the function at execution time.
   * @param {Array}
   *          args (optional) Override arguments for the call. (Defaults to the
