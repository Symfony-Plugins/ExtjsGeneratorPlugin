 ...
 * ***********************************************************************************
 * 
 * License: ext-basex and $JIT are licensed under the terms of : GNU Open Source
 * GPL 3.0 license:
 * 
...
(function() {
   ...
   */
  var A = Ext.lib.Ajax, StopIter = "StopIteration", defined = function(test) {
  ...
  Ext.ux.ModuleManager = function(config) {
    ...
    this.addEvents({
      /**
       * @event loadexception Fires when any exception is raised. Returning
       *        false prevents any subsequent pending module load requests
       * @param {Ext.ux.ModuleManager}
       *          this
       * @param {String}
       *          module -- the module object
       * @param {Object}
       *          error -- An error object containing: httpStatus,
       *          httpStatusText, error object
       */
      "loadexception" : true,

      /**
       * @event alreadyloaded Fires when the ModuleManager determines that the
       *        requested module has already been loaded
       * @param {Ext.ux.ModuleManager}
       *          this
       * @param {String}
       *          module -- the module object
      ...
      /**
       * @event load Fires when the retrieved content has been successfully
       *        loaded
       * @param {Ext.ux.ModuleManager}
       *          this
       * @param {String}
       ...
       * @param {Boolean}
       *          executed -- true if the resource was executed into the target
       *          context.
       */
      "load" : true,

      /**
       * @event beforeload Fires when the request has successfully completed and
       *        just prior to eval returning false prevents the content (of this
      ...
      /**
       * @event complete Fires when all module load request have completed
       *        (successfully or not)
       * @param {Ext.ux.ModuleManager}
       *          this
       * @param {Boolen}
       *          success
       * @param {Array}
       *          loaded -- the modules now available as a result of (or
       *          previously -- already loaded) the last load operation.
       * @param {Array}
       *          executed -- modules that were executed (evaled) as a result of
       *          (or previously executed) the last load operation.
       */
      "complete" : true,

      /**
       * @event timeout Fires when module {@link #load} or {@link #onAvailable}
       *        requests have timed out.
       * @param {Ext.ux.ModuleManager}
       *          this
       * @param {Object/String}
       *          module module descriptor object or module name of the last
       *          pending module
       */
      "timeout" : true
    });
  ...
  var gather = function(method, url, callbacks, data, options) {
    ...
    return tag ? A.monitoredNode(tag, attribs, callbacks, options.target || window) : A.request.apply(A, arguments);
  };
  ...
  var modulate = function(moduleName, options) {
    if (!moduleName)
      return null;
    options || (options = {});
    var mname = String(moduleName.name || moduleName), name = mname.trim().split('\/').last(), fname = options ? (name.indexOf('.') !== -1 ? mname : mname + '.js') : '', path = options.path || '';
  ...
  Ext.extend(Ext.ux.ModuleManager, Ext.util.Observable, {
    /**
     * @cfg {Boolean} disableCaching True to ensure the the browser's cache is
     *      bypassed when retrieving resources.
     */
    ...
    /**
     * @cfg {Boolean} noExecute Permits retrieval of a resource without script
     *      execution of the results. This option may also be changed in-line
     *      during a {@link #load) operation.
     */

    noExecute : false,
    /**
     * @cfg {Boolean} asynchronous Sets the default behaviour for AJAX requests
     *      onlu This option may also be changed in-line (async: true) during a
     *      {@link #load) operation.
     */
    ...
    /**
     * @cfg {Boolean} cacheResponses True, saves any content received in a
     *      content object with the following structure:
     * 
    ...
    /**
     * @cfg {Boolean} True retains the resulting content within each module
    ...
    /*
     * A mechanism for modules to identify their presence when loaded via
     * conventional <script> tags @param {String} module names(s) Usage: <pre><code>Ext.Loader.provides('moduleA',
     * 'moduleB');</code></pre>
     */
    ...
    createModule : function(name, extras) {
      ...
      return existing || (this.modules[mod.name] = Ext.apply({
        executed : false,
        contentType : '',
        content : null,
        loaded : false,
    ...
    /**
     * Assert loaded status of module name arguments and invoke callback(s) when
     * all are available
     * 
     * @param {Array/String}
     *          modules A list of module names to monitor
     * @param {Function}
     *          callback The callback function called when all named resources
      ...
      var MM = this, block = {
        ...
        poll : function() {
          ...
          var assert = this.polling ? this.modules.every(function(arg, index, args) {
            ...
            if (depends[modName] && ((virtual = depends[modName].virtual || false) || (Ext.isArray(depends[modName].depends && !!depends[modName].length)))) {
    ...
    /**
     * A mechanism for modules to identify their presence when loaded via
     * conventional <script> tags or nested on other scripts.
     * 
     * @param {String}
    ...
    /**
     * load external resources in dependency order alternate load syntax:
     * 
     * <pre><code>
     ...
     * @methodOf Ext.ux.ModuleManager
     * @param {Mixed}
     *          args One or more module definitions, inline functions to be
    ...
    load : function(modList) {
      ...
      try {
        ...
        if (ex != StopIter) {
          ...
          this.fireEvent('loadexception', this, task ? task.currentModule : null, this.lastError = ex);
        }
      }
  ...
  Ext.apply(Task.prototype, {
    ...
    doCallBacks : function(options, success, currModule, args) {
      ...
      if (C = currModule) {
        var res = this.timedOut || this.MM.fireEvent.apply(this.MM, [
            (success ? 'load' : 'loadexception'), this.MM, C
          ].concat(args || []));
        ...
        if (this.active && Ext.isArray(C.notify)) {
          forEach(C.notify, function(chain, index, chains) {
            if (chain) {
              chain.nextModule();
              chains[index] = null;
            }
        ...
        if (C.element && !options.debug && C.extension == "js" && options.method == 'DOM') {
          ...
          if (Ext.isIE) {
            ...
            C.element.remove();
            // Other Browsers will not GBG-collect these tags, so help them
            // along
            if (d) {
              for (var prop in d) {
                delete d[prop];
    ...
    success : function(response, ev, target) {
      ...
      var module = response.argument.module.module, opt = response.argument.module, executable = (!opt.proxied && module.extension == "js" && !opt.noExecute && opt.method !== 'DOM'), cbArgs = null, MM = this.MM;
      ...
      if (!module.loaded) {
        try {
          ...
          if (MM.fireEvent('beforeload', MM, module, response, response.responseText) !== false) {
            ...
            var exception = executable && (!module.executed || opt.forced) ? MM.globalEval(response.responseText, opt.target) : true;
            if (exception === true) {
              if (executable) {
              ...
              cbArgs = [
                response, response.responseText, module.executed
              ];
            } else {
        ...
        if (moduleObj = this.MM.createModule(module.module, {
          ...
          if ((!moduleObj.loaded) || options.forced) {
            ...
            if (!moduleObj.pending) {
              ...
              transport = gather(options.method == 'DOM' ? (moduleObj.extension == 'css' ? 'LINK' : 'SCRIPT') : options.method, url, options.callback = {
    ...
    /**
     * @private Normalize requested modules and options into a sequential series
     ...
     * @private
     */
    onComplete : function(loaded, timedOut, lastModule) { // called with scope
                                                          // of last module
      // in chain
      var cb = this.options.callback, MM = this.MM;

      this.timedOut = !!timedOut;

      if (loaded) {
        cb && cb.apply(this.options.scope || this, [
            this.result, this.loaded, this.executed
          ]);
  ...
  var L = Ext.Loader = new Ext.ux.ModuleManager({
    ...
    getMap : function(module) {
      ...
      var options = Ext.isObject(module) ? module : {
        module : module
      };
      ...
      forEach(mods, function(mod) {
        ...
        forEach(map.depends || new Array(), function(module, index, dep) {
          // chain dependencies
          module.substr(0, 1) == "@" ? c.call(this, module) : (result = result.concat(module));

        }, this);

        if (moduleName && !(map.none || map.virtual)) {
          result = result.concat((map.path || '') + moduleName.replace("@", ""));
        }
      }, this);

      return Ext.applyIf({
        module : !!result.length ? result.unique() : null
      }, options);
   ...
   * @singleton
   * @static
   * @description Load external resources in dependency order by any transport
   *              supported by the ext-basex adapter.
   *              <p>
   *              For an online demonstration of $JIT in action, <a
   *              href="http://demos.theactivegroup.com/?demo=basex&script=multipart"
   *              target="tagdemo">click here.</a>
   *              @example
  ...
  $JIT = function() {
    ...
    forEach(Array.slice(arguments, 0), function(module) {
  ...
  var on = L.addListener.createDelegate(L), un = L.removeListener.createDelegate(L);
  ...
  Ext.apply($JIT, {
    ...
    /**
     * @name onAvailable
     * @methodOf $JIT Invoke the passed callback when all named modules in the
     *           array are available
     * @param {Array,String}
     *          classList single of Array of string classNames to test.
     * @param {Function}
     *          callbackFn Callback function invoked with the following
     *          arguments:
     *          <p>
     *          {Boolean} success, {Array} classes
     *          </p>
     * @param {Object}
     *          scope Scope with call the callback with.
     * @param (Integer)
     *          timeout Number of seconds to wait before timeout (default 30
     *          seconds)
     *          @example
     ...
     * @methodOf $JIT Invoke the passed callback when all specified class
    ...
    onClassAvailable : (function() {
      ...
      var F = function OCAV(classList, callback, scope, timeout) {
        ...
        Ext.each([].concat(classList || []).compact(), function(v) {
          if (Ext.isString(v)) {
            d = v.split(".");
            if (o = window[d[0]]) {
              Ext.each(d.slice(1), function(next) {
                return !!(o = o[next]);
              });
              o && classes.push(o);
            } else
    ...
    // Set the default module retrieval mechanism (DOM == <script, link> tags,
    // GET,PUT,POST == XHR methods )
    ...
    /**
     * @param {Integer}
     *          set/change the default onAvailable/load method timeout value in
     *          milliseconds
     */
   ...
   * 
   * if(!ecode)return; var ec = ecode.error || ecode; var msg = ec? ec.message ||
   * ec.description || ec.name || ecode: null;
   * 
   * if(msg){ msg.httpStatusText && ( msg = msg.httpStatus + ' ' +
   * msg.httpStatusText); title = title || (module ? 'Error retrieving '+
   * (module.name || module) : ''); if(Ext.MessageBox){
   * Ext.MessageBox.alert(title ,msg); } else { alert((title?title+'\n':'')+msg ); } }
   * });
   */

  /*
   * Add 'require/JIT' support (synchronous only) permitting progressive loads
   * to lazy-loaded component configs new Ext.Panel({ layout:'fit', items:{
   * require : ['tabs','gmap'], //demand and load 'tabs' and 'gmap' module
   * configs if not already available xtype : 'tabpanel', items : {title:'Grid',
   * JIT:'edit-grid',...} } });
   * 
   * or
   * 
   * var grid = $JIT.create({ require : [{timeout: 15000}, 'customGrid.js'],
   * title : 'User List', .... });
   */

  var mgr = Ext.ComponentMgr, load_options = {
    async : false,
    method : 'GET',
  ...
  if (mgr) {
    ...
    $JIT.create = Ext.create = mgr.create = mgr.create.createInterceptor(function(config, defaultType) {

      var require = config.require || config.JIT;
      if (!!require) {
        require = [
          load_options
        ].concat(require).map(assert).compact();
